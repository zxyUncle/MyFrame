...
/**
 * Created by zsf on 2022/8/29 13:51
 * ******************************************
 * *
 * ******************************************
 */
...
class MyHttpLoggingInterceptor @JvmOverloads constructor(
    ...
    fun setListFilter(listFilter: MutableList<String>) = apply {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        //做日志过滤，部分指定的日志不需要打印日志
        try {
            listFilter.map {
                if (request.url.toUrl().path.contains(it)) {
                    return chain.proceed(request)
                }
            }
        } catch (e: Exception) {
